...
4. The response must be HTTP 200, `application/json`, at most `max_document_bytes` (32 KB), and valid JSON with a `client_id` that matches the fetched URL exactly. `client_name` and `redirect_uris` are required. A metadata document is published once for every authorization server its client talks to, so it routinely advertises capabilities this one does not implement. Grant types other than `authorization_code` and `refresh_token`, response types other than `code`, and token endpoint authentication methods other than `none` are therefore dropped rather than treated as a fatal document error — the registration keeps the ones that work, and is refused only when nothing usable is left, meaning no `authorization_code` grant, no `code` response type, or no `none` authentication method (the singular `token_endpoint_auth_method` preference and the `token_endpoint_auth_methods_supported` list are both considered). Dropping them concedes nothing, because the token endpoint dispatches from its own allowlist rather than from what a client registered. The same sanitization limits as dynamic registration apply, and the `oauth_pilot__client_registration_metadata` filter runs over the normalized document.
...
A route covered by a more specific registered resource is left to that resource. Authentication resolves the deepest registered audience for the request URL, and when the audience it finds is not the shared REST one — an MCP server nested under `wp-json` that registers its own scopes, for example — this layer stands aside entirely, so the route authenticates its own tokens against its own audience. Without that, a token minted for such a route would be measured against the shared `wp-json` audience and refused as issued for a different resource before the route ever ran, and a token for the shared audience would be accepted on a route that never belonged to it. Use the `oauth_pilot__rest_authentication_resource_uri` filter to select a different audience for a route.
...
**A client with a URL-shaped client_id fails with `invalid_client`.** That is the Client ID Metadata Document path: the document at the client_id URL could not be fetched, was not JSON, did not match the URL, was served from a private address, or advertised no authentication method this server can run. Open the URL in a browser, make sure the WordPress host can reach it (outbound HTTP, DNS), and check the `cimd_fetch_failed` security event for the reason. ChatGPT's document prefers `private_key_jwt` but also lists `none`; that combination is accepted. A document that only supports `private_key_jwt` is still refused, because this server does not verify client assertions. A client that worked for months and suddenly fails has usually let its document's certificate or hosting lapse — the cached copy is dropped once it is more than 7 days past expiry.
